...
class PIIRedactor:
    ...
    def __init__(self):
        ...
        self.account_pattern = re.compile(
            r'\b(?:account|cuenta|número|number)[\s#:]*([0-9]{6,})\b',
            re.IGNORECASE
        )
    ...
    def redact_text(self, text: str, preserve_context: bool = True) -> Tuple[str, List[str]]:
        """
        ...
        for match in self.account_pattern.finditer(redacted):
            ...
            if preserve_context and len(account_num) > 4:
                masked_account = account_num[:2] + "*" * (len(account_num) - 4) + account_num[-2:]
    ...
    def redact_headers(self, headers: str) -> str:
        ...
        for line in lines:
            ...
            if line.lower().startswith(('x-forwarded-for:', 'x-real-ip:', 'x-client-ip:')):
    ...
    def extract_safe_snippets(self, text: str, max_length: int = 500) -> List[str]:
        ...
        security_keywords = [
            'urgent', 'urgente', 'immediate', 'inmediato', 'verify', 'verificar',
            'account', 'cuenta', 'password', 'contraseña', 'click', 'clic',
            'suspended', 'suspendida', 'expired', 'expirada', 'update', 'actualizar'
        ]
