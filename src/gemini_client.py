...
class GeminiClient:
    ...
    async def classify_email(
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                logger.error(
                    f"Gemini error on attempt {attempt + 1}: {type(e).__name__}: {e}"
                )
                if attempt < self.max_retries:
                    await asyncio.sleep(0.1 + random.uniform(0, 0.2))  # Jitter
                continue
    ...
    async def _make_gemini_request(
        ...
        try:
            ...
            # Generate response
            logger.info(
                f"Calling Gemini 2.5 Pro with prompt length: {len(full_prompt)}"
            )
            ...
            if response.candidates and len(response.candidates) > 0:
                ...
                if hasattr(candidate, "content") and candidate.content:
                    if (
                        ...
                        logger.warning(
                            f"Gemini response has no parts: {candidate.content}"
                        )
                else:
