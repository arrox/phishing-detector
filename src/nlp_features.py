...
class NLPAnalyzer:
    ...
    def __init__(self):
        ...
        self.brand_entities = {
            'financial': [
                'paypal', 'amazon', 'ebay', 'mercadolibre',
                'santander', 'bbva', 'caixabank', 'ing',
                'scotia', 'citibank', 'hsbc', 'chase'
            ],
            'tech': [
                'microsoft', 'apple', 'google', 'facebook',
                'netflix', 'spotify', 'adobe', 'zoom'
            ],
            'government': [
                'hacienda', 'irs', 'social security', 'seguridad social'
            ]
        }
    ...
    def analyze_text(self, text_content: str) -> Tuple[NLPSignals, List[str]]:
        ...
        if signals.urgency_score > 0.3:
            signal_descriptions.append(f"Urgencia detectada (score: {signals.urgency_score:.2f})")
        ...
        if signals.brand_mentions:
            signal_descriptions.append(f"Marcas mencionadas: {', '.join(signals.brand_mentions[:3])}")
    ...
    def _check_payment_request(self, text: str) -> bool:
        ...
        financial_words = ['bank', 'card', 'payment', 'money', 'banco', 'tarjeta', 'pago']
        action_words = ['update', 'verify', 'confirm', 'provide', 'actualizar', 'verificar']
    ...
    def _check_language_mixing(self, text: str) -> bool:
        ...
        spanish_matches = sum(1 for pattern in spanish_indicators 
                            if re.search(pattern, text, re.IGNORECASE))
        english_matches = sum(1 for pattern in english_indicators 
                            if re.search(pattern, text, re.IGNORECASE))
    ...
    def _extract_threat_indicators(self, text: str) -> List[str]:
        ...
        additional_threats = [
            'cuenta bloqueada', 'account blocked', 'suspended',
            'verificación requerida', 'verification required',
            'actividad sospechosa', 'suspicious activity'
        ]
