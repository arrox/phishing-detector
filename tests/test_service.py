...
class TestPhishingDetectionService:
    ...
    @pytest.mark.asyncio
    async def test_classify_phishing_email(self, detection_service, sample_phishing_request, mock_gemini_client):
    ...
    @pytest.mark.asyncio
    async def test_classify_legitimate_email(self, detection_service, sample_legitimate_request, mock_gemini_client):
        ...
        mock_response = ClassificationResponse(
            ...
            recommended_actions=["El mensaje parece seguro", "Mantén precauciones generales"],
            evidence=Evidence(
    ...
    @pytest.mark.asyncio
    async def test_classify_suspicious_email(self, detection_service, sample_suspicious_request, mock_gemini_client):
    ...
    @pytest.mark.asyncio
    async def test_gemini_failure_fallback(self, detection_service, sample_phishing_request, mock_gemini_client):
    ...
    @pytest.mark.asyncio
    async def test_security_policy_elevation(self, detection_service, sample_phishing_request, mock_gemini_client):
        ...
        mock_response = ClassificationResponse(
            ...
            evidence=Evidence(
                header_findings=HeaderFindings(spf_dkim_dmarc="fail"),  # Critical signal
                url_findings=[],
    ...
    @pytest.mark.asyncio
    async def test_attachment_analysis(self, detection_service, sample_attachment_request, mock_gemini_client):
        """Test analysis of suspicious attachments."""
        mock_response = ClassificationResponse(
            classification="phishing",
            risk_score=85,
            top_reasons=["Suspicious attachments", "Executable files", "Unknown sender"],
    ...
    @pytest.mark.asyncio
    async def test_latency_within_slo(self, detection_service, sample_phishing_request, mock_gemini_client):
    ...
    async def test_error_handling(self, detection_service, sample_phishing_request):
        ...
        with patch.object(detection_service, '_run_heuristic_pipeline', side_effect=Exception("Test error")):
            result = await detection_service.classify_email(sample_phishing_request)
